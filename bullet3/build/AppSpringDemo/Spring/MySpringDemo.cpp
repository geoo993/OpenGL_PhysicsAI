...
Spring::Spring(btRigidBody* a, btRigidBody* b, btScalar hooke, btScalar damp) :
	bodyA(a), bodyB(b),
	length((a->getWorldTransform().getOrigin() -
		b->getWorldTransform().getOrigin()).length()),
	hooke_constant(hooke), damping(damp) {}

void Spring::applyForce()
{
	btVector3 posA = bodyA->getWorldTransform().getOrigin();
	btVector3 posB = bodyB->getWorldTransform().getOrigin();
	btVector3 diff = posB - posA;
	btVector3 velA = bodyA->getLinearVelocity();
	btVector3 velB = bodyB->getLinearVelocity();
	btVector3 vdiff = velB - velA;
	btScalar forceSpring = hooke_constant*(diff.length() - length);
	btScalar forceDamping = damping*vdiff.dot(diff.normalized());
	btVector3 force = (forceSpring+forceDamping)*diff.normalized();
...
void Spring::draw()
{
	...
	glVertex3d(posA.getX(), posA.getY(), posA.getZ());
	glVertex3d(posB.getX(), posB.getY(), posB.getZ());
}

SpringNetwork::SpringNetwork(btScalar hooke, btScalar damp) :
	hooke_constant(hooke), damping(damp) {}

void SpringNetwork::addSpring(btRigidBody* a, btRigidBody* b) {
	spring.push_back(Spring(a, b, hooke_constant, damping));
}

void SpringNetwork::draw() {
	glBegin(GL_LINES);
...
void SpringNetwork::applyForce() {
	for (int i = 0; i < spring.size(); i++)
		spring[i].applyForce();
}

void myTickCallback(btDynamicsWorld *world, btScalar timeStep) {
	SpringNetwork *springs = static_cast<SpringNetwork *>(world->getWorldUserInfo());
	springs->applyForce();
}

MySpringDemo::MySpringDemo() : springs(700.0f, 4.0f) {}

void MySpringDemo::initPhysics()
{
	setTexturing(true);
	setShadows(false);
...
//	wp->getSolverInfo().m_erp = 0.8;
	m_dynamicsWorld->setInternalTickCallback(myTickCallback, static_cast<void *>(&springs), true);

	{ // floor
		btCollisionShape* groundShape = new btBoxShape(btVector3(btScalar(500.),btScalar(1.),btScalar(500.)));
		m_collisionShapes.push_back(groundShape);
		btTransform groundTransform;
		groundTransform.setIdentity();
		groundTransform.setOrigin(btVector3(0,-31,0));
...

	{
		btTransform trans;
		trans.setIdentity();
		btCollisionShape* shape = new btBoxShape(btVector3(btScalar(6.),btScalar(2.),btScalar(20.)));
		m_collisionShapes.push_back(shape);
		trans.setOrigin(btVector3(-11, -28, 6));
		localCreateRigidBody(0, trans, shape);
		trans.setOrigin(btVector3(15, -28, 6));
		localCreateRigidBody(0, trans, shape);
	}

	{
		...
		btTransform trans;
		trans.setIdentity();
		btCollisionShape* shape = new btSphereShape(5.0);
		m_collisionShapes.push_back(shape);
		trans.setOrigin(btVector3(0, 40, 6));
	...
	{ // spheres
		...
		const btScalar spacing = 4.0f;
		const btVector3 corner(-6, -16, -4);

		btScalar mass = btScalar(5.);
		btTransform trans;
		trans.setIdentity();
		btCollisionShape* shape = new btSphereShape(radius);
		m_collisionShapes.push_back(shape);

		btRigidBody* sphere[xside][yside][zside];
		for (int i = 0; i < xside; i++)
			for (int j = 0; j < yside; j++)
				for (int k = 0; k < zside; k++) {
					trans.setOrigin(corner + spacing*btVector3(i, j, k));
					sphere[i][j][k] = localCreateRigidBody(mass, trans, shape);
				}

		for (int i = 0; i < xside; i++)
			for (int j = 0; j < yside; j++)
				for (int k = 0; k < zside; k++) {
					btRigidBody* here = sphere[i][j][k];
					if (i > 0)
						springs.addSpring(sphere[i-1][j][k], here);
					if (j > 0)
						springs.addSpring(sphere[i][j-1][k], here);
					if (k > 0)
						springs.addSpring(sphere[i][j][k-1], here);
					if (i > 0 && j > 0)
						springs.addSpring(sphere[i-1][j-1][k], here);
					if (j > 0 && k > 0)
						springs.addSpring(sphere[i][j-1][k-1], here);
					if (k > 0 && i > 0)
						springs.addSpring(sphere[i-1][j][k-1], here);
					if (i > 0 && j < yside-1)
						springs.addSpring(sphere[i-1][j+1][k], here);
					if (j > 0 && k < zside-1)
						springs.addSpring(sphere[i][j-1][k+1], here);
...
void MySpringDemo::clientMoveAndDisplay()
{
	...
	glFlush();
	glutSwapBuffers();
} // MySpringDemo::clientMoveAndDisplay()

void MySpringDemo::displayCallback(void) 
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); 
	if (m_dynamicsWorld)
	{
	...
	glFlush();
	glutSwapBuffers();
} // MySpringDemo::displayCallback()

void MySpringDemo::keyboardCallback(unsigned char key, int x, int y)
{
	switch (key) 
	{
		default : 
...
//	spHingeConst->setLimit(-1.57, 1.57);
	...
	// btCollisionShape* shape = new btCapsuleShapeX(radius, length);
	// btCollisionShape* shape = new btSphereShape(radius);

#if 1 // Creation of executive toy
	{
		{
			// add box shape (will be reused for all bodies)
		...
		int numberSpheres = 5;
		float radius = 1;
		btVector3 anchorOffset(radius*2, 0, 0);
		trans.setIdentity();
		btVector3 origin(numberSpheres*radius, -2, -10);
		trans.setOrigin(origin);
		
		for (int i=0; i<numberSpheres; ++i) {
			shape = new btSphereShape(radius);
			m_collisionShapes.push_back(shape) ;
			body = localCreateRigidBody(mass, trans, shape) ;
			joint = new btPoint2PointConstraint(*body, btVector3(0, 20, 0)) ;
			m_dynamicsWorld->addConstraint(joint, true ) ;
			origin += anchorOffset;
			trans.setOrigin(origin);
		}
		// Add ball to swing in
		shape = new btSphereShape(radius);
		m_collisionShapes.push_back(shape);
		origin += btVector3(10, 10, 0);
		trans.setOrigin(origin);
		body = localCreateRigidBody(mass, trans, shape);
		joint = new btPoint2PointConstraint(*body, btVector3(-10, 0, 0)) ;
		m_dynamicsWorld->addConstraint(joint, true ) ;
	}
#endif

#if 1 // Creation of chain of sausages
	{
		btScalar mass = 1;
		btTransform trans;
		trans.setIdentity();
		trans.setOrigin(btVector3(0 , 5, 5) ) ;
		float radius = 0.2;//1;
		float length = 0.6;//3;
		btVector3 halfExt(-((length+2*radius)/2),0,0);
		btCollisionShape* shape = new btCapsuleShapeX(radius, length);
		m_collisionShapes.push_back(shape) ;
		btRigidBody* body =
			localCreateRigidBody(mass, trans, shape) ;
		btPoint2PointConstraint* joint =
			new btPoint2PointConstraint(*body, -halfExt) ;
		m_dynamicsWorld->addConstraint(joint, true ) ;

		//trans.setIdentity();
		//trans.setOrigin(btVector3(-5 , 5, 0) ) ;
		btRigidBody* body2;
		for (int i=0; i<25; ++i) {

			trans.setOrigin(trans.getOrigin()+halfExt+halfExt);
			shape = new btCapsuleShapeX(radius, length);
			m_collisionShapes.push_back(shape) ;
			body2 = localCreateRigidBody(mass, trans, shape) ;
			joint =	new btPoint2PointConstraint(*body, *body2, halfExt, -halfExt) ;
			m_dynamicsWorld->addConstraint(joint, true ) ;
			body = body2;
		}

		joint = new btPoint2PointConstraint(*body2, halfExt) ;
		m_dynamicsWorld->addConstraint(joint, true ) ;
	}
#endif
